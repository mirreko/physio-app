<template>
  ...
  try {
    ...
    localStorage.setItem('token', data.token); // Speichere das Token für die spätere Authentifizierung

    // Um Benutzerrolle abfragen
    const userResponse = await fetch('http://localhost:5500/api/auth/user', {
      headers: {
    ...
    this.errorMessage = 'Anmeldung fehlgeschlagen. Bitte überprüfen Sie Ihre Daten.';
  }
},
